...
class alertable_event
{
    HANDLE ev{};

  public:
    alertable_event() noexcept(false);
    ~alertable_event() noexcept;
    alertable_event(const alertable_event&) = delete;
    alertable_event(alertable_event&&) = delete;
    alertable_event& operator=(const alertable_event&) = delete;
    alertable_event& operator=(alertable_event&&) = delete;
...
alertable_event::alertable_event() noexcept(false)
...
alertable_event::~alertable_event() noexcept
{
    this->close();
}
bool alertable_event::is_closed() const noexcept
{
    return ev == INVALID_HANDLE_VALUE;
}
void alertable_event::close() noexcept
{
    CloseHandle(ev);
    ev = INVALID_HANDLE_VALUE;
}
void alertable_event::reset() noexcept
{
    ResetEvent(ev);
}
void alertable_event::set() noexcept
{
    SetEvent(ev);
}
bool alertable_event::wait(uint32_t ms) noexcept
{
    static_assert(WAIT_OBJECT_0 == 0);

    // This makes APC available. expecially for Overlapped I/O
    if (WaitForSingleObjectEx(ev, ms, TRUE))
...
// this cast doesn't harm semantics
#define DECLARE_LATCH_MEMBERS()                                                \
    auto ev = (alertable_event*)(storage + 0);                                 \
    auto ctr = (atomic_uint64_t*)(storage + sizeof(*ev));

latch::latch(uint32_t delta) noexcept(false) : storage{}
{
    DECLARE_LATCH_MEMBERS();

    if (new (ctr) atomic_uint64_t{})
        ctr->fetch_add(delta);

    if (new (ev) alertable_event{})
        ev->reset();
}
GSL_SUPPRESS(con .4)
...
void latch::count_down(uint32_t n) noexcept(false)
{
    ...
    if (ctr->load(memory_order_acquire) < n)
        throw underflow_error{"latch's count can't be negative"};

    ctr->fetch_sub(n, memory_order_release);
    if (ctr->load(memory_order_acquire) == 0)
        ev->set();
}
GSL_SUPPRESS(f .23)
GSL_SUPPRESS(con .4)
