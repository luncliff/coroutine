    ...
    -> stdex::generator<endpoint>
{
    ...
    // Success : zero
    // Failure : non-zero uint32_t code
     uint32_t ec = ::getaddrinfo(name, serv,
                                  std::addressof(hints), &list);
    if (ec != NO_ERROR)
        co_return;
...
//      Coroutine resolver
auto resolve(addrinfo hints,
             const char *name, const uint16_t port) noexcept
    -> stdex::generator<endpoint>
{
...
auto resolve(const char *name, uint16_t port) noexcept
    -> stdex::generator<endpoint>
{
    addrinfo hints = hint(AI_ALL | AI_V4MAPPED | AI_NUMERICSERV,
                          SOCK_STREAM);
...
auto resolve(uint16_t port) noexcept
    -> stdex::generator<endpoint>
{
    addrinfo hints = hint(AI_ALL | AI_V4MAPPED | AI_NUMERICSERV,
                          SOCK_STREAM);
