...
TEST_CLASS(SocketTest)
{
    ...
    TEST_METHOD(ResolveToConnect)
    {
        ...
        while (iter != nullptr)
        {
            ...
            {
                sockaddr_in *v4a = reinterpret_cast<sockaddr_in *>(iter->ai_addr);
    ...
    TEST_METHOD(ResolveToAccept)
    {
        ...
        addrinfo hint = make_hint(AI_PASSIVE | AI_V4MAPPED, AF_INET6, SOCK_STREAM);
        addrinfo *list = nullptr;

        // Success : zero
        // Failure : non-zero uint32_t code
        e = ::getaddrinfo(name, serv, std::addressof(hint), std::addressof(list));
    ...
    TEST_METHOD(NameOfEndpoint)
    {
        ...
        // Success : zero
        // Failure : non-zero uint32_t code
        e = ::getnameinfo(ptr, sizeof(sockaddr_in6), name_buf, NI_MAXHOST, serv_buf,
    ...
    {
        error_t e{};
        SOCKET sd = ::WSASocketW(AF_INET6, SOCK_RAW, IPPROTO_RAW, nullptr, 0,
                                 WSA_FLAG_OVERLAPPED);
        e = WSAGetLastError();
